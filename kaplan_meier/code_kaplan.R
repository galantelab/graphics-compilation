...
pdf("kaplan_LCK.pdf")
pp = suppressWarnings(ggsurvplot(fit, risk.table=T, pval=T, censor=T, break.time.by=200, 
...
#Plot Kaplan-Meier curve
pdf("kaplan_LCK_gender.pdf",width=9)
pp = suppressWarnings(ggsurvplot(fit, risk.table=T, pval=T, censor=T, break.time.by=200, 
           legend.title="", font.legend=c(14,"bold"), font.x=c(18, "bold"), 
           font.y=c(18, "bold"), font.tickslab=16, pval.size=6, pval.coord=c(0,0.05), legend = c(0.8, 0.9)) + 
           xlab("Survival time (days)"))
print(pp, newpage = FALSE)
garbage=dev.off()
